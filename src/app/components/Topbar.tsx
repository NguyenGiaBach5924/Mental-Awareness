...
export function Topbar({ darkMode, toggleDarkMode, language, toggleLanguage, toggleSidebar }: TopbarProps) {
  return (
    <div className="fixed top-0 left-0 right-0 h-[60px] bg-[#1E2A38] z-50 flex items-center px-4 md:px-6">
      <button
        onClick={toggleSidebar}
        className="md:hidden text-white mr-4 hover:bg-white/10 p-2 rounded"
      >
        <Menu size={24} />
      </button>

      <div className="flex-shrink-0 text-white mr-8">
        <h1 className="text-lg md:text-xl">Mental Health Awareness</h1>
      </div>
